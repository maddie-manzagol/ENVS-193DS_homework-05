...
a. We found that the full_log model including species, feedlevel, sla, chlorophyll, amass, num_lvs, and num_phylls predictors best predicted totmass because the full_log model yields the lowest AIC value indicating that this model has the best predictive capacity while remaining as simple as possible. Overall, the full_log model was statistically significant (R^2= 0.8687, F(15, 87)= 38.38, p<0.001).  

b. With all else held constant, the full_log model indicates a significant relationship between totmass and num_lvs (p< 0.001). Comparing to the alabamensis species of pitcher plants, the leucophylla species are likely to have a biomass that is 1.7 higher (95% confidence level, [1.25, 2.16]) and the psittacina species are likely to have a biomass that is 1.6 lower (95% confidence level, [-2.05,-1.23]). However it was found that some of the predictors such as, the jonesii and purpurea species, amass, and num_phylls, did not significantly predict totmass (p= 0.107, p= 0.155, p= 0.436, p= 0.446, respectively).
...
#does the same thing as above, but different code style 
