...
a.  Sampling Methods (these should be summarized from the metadata and
    cited)
...
site: Harvard Forest Greenhouse (Ellison and Farnsworth). To conduct the study,
plants were fed once a week for 7 weeks straight (Ellison and Farnsworth). Two plants
of each Sarracenia species were assigned to one of six feeding levels in
a regression design ranging from 0 - 0.25g of finely ground wasps per
feeding (for small species), 0 - 0.5g (for species of intermediate
sizes), and 0 - 1.0g (for large species) (Ellison and Farnsworth). "Aboveground size"
and "Amass" were measured prior to commencing treatments, meanwhile all
other parameters were measured during the duration of the study (Ellison and Farnsworth).
...
c.  Visualization of missing observations with an accompanying caption
    and discussion (caption: 1-3 sentences, discussion: 1-3 sentences)
...
that one can identify where bias within statistical analysis might
...
d.  Visualization of Pearson's correlation between variables with an
    accompanying caption and discussion (caption: 1-3 sentences,
...
e.  Visualization of relationships between variables (not correlation)
    with an accompanying caption and discussion (caption: 1-3 sentences,
    discussion: 1-3 sentences)
...
g.  Visual and statistical assumption checks for the full model with
    accompanying discussion (1-3 sentences)
...
variable of interest and achieve normality, we applied a log transformation. Once we did this
we were able to manipulate the residuals in a manner that reported
normal distribution and homoscedastic variance.
...
i.  Model construction with visual and statistical assumption checks for
    three additional models with justification (i.e. why did you choose
    those predictor variables?) (justification: 1-3 sentences per model)
...
variables species, feedlevel, and num_lvs. The species variable was chosen as a predictor variable because it is probable that different species have different biomasses. The model comparing species as predictor variable and totmass as the response variable was found to
be normal and homoscedastic. The feedlevel variable was chosen as a predictor variable because different feed levels are associated with different nutrient level. Different nutrient levels are indicative of growth and overall health which contributes to individual's biomass. The num_lvs variable was chosen as a predictor variable because the more leaves an individual has, the more biomass an individual has. The models comparing feedlevel and num_lvs to predict totmass, respectively, both were found to be non-normal, yet homoscedastic. 
...
j.  Variance inflation factor check for the full model with accompanying
    discussion (discussion: 1-3 sentences)
...
a.  We found that the full_log model including species, feedlevel, sla,
    chlorophyll, amass, num_lvs, and num_phylls predictors best
    predicted totmass because the full_log model yields the lowest AIC
    value indicating that this model has the best predictive capacity
    while remaining as simple as possible. Overall, the full_log model
    was statistically significant (R\^2= 0.8687, F(15, 87)= 38.38,
    p\<0.001).

b.  With all else held constant, the full_log model indicates a
    significant relationship between totmass and num_lvs (p\< 0.001).
    Comparing to the alabamensis species of pitcher plants, the
    leucophylla species are likely to have a biomass that is 1.7 higher
    (95% confidence level, \[1.25, 2.16\]) and the psittacina species
    are likely to have a biomass that is 1.6 lower (95% confidence
    level, \[-2.05,-1.23\]). However it was found that some of the
    predictors such as, the jonesii and purpurea species, amass, and
    num_phylls, did not significantly predict totmass (p= 0.107, p=
    0.155, p= 0.436, p= 0.446, respectively).
...
79: 880-898.
https://doi.org/10.1890/0012-9658(1998)079\[0880:IBIFWS\]2.0.CO;2 (Accessed 2023-06-04).

  Ellison, A. and E. Farnsworth. 2021. Effects of Prey Availability on Sarracenia Physiology at Harvard Forest 2005 ver 18. Environmental Data Initiative. https://doi.org/10.6073/pasta/26b22d09279e62fd729ffc35f9ef0174 (Accessed 2023-06-04). 
...
https://doi.org/10.1111/j.1365-2745.2007.01313.x (Accessed 2023-06-04).
...
https://doi.org/10.1674/0003-0031-167.1.13 (Accessed 2023-06-04). 
